...
####13.3 adds:
####   random fix to the duplicate script where it was reading empty dup_test_sites dataframe as having a row when determining whether to run the loop around line 347
####13.4 adds:
...
for (m in 1:nrow(sort_sites2)){ 
  ...
  next_buffer_sf_aeqd <- st_transform(next_buffer_sf, crs = st_crs(aeqd_proj))
  sort_sites2_sf_aeqd <- st_transform(sort_sites2_sf, crs = st_crs(aeqd_proj))
...
while (nrow(sites) > 0){
  ...
  lookup_buffer_sf <- sf::st_buffer(lookup_site_sf, units::set_units(lookup_site$Radius, degree)) %>% #Create an sf object of the buffer of the lookup_site by its radius
    st_transform(crs = st_crs(aeqd_proj)) #after running the degree buffer, project it to a meter-level CRS
  ...
  sites_sf <- sf::st_as_sf(sites, coords = c("Long","Lat"), crs = st_crs(4326)) %>% #create an sf object of all sites... needs to be inside the loop so the sites available for intersection is updated each time
    ...
    st_transform(crs = st_crs(aeqd_proj)) #project it to a meter-level CRS
    ...
    if (nrow(long_routes) > 0) {
      ...
      for (q in long_route_sites_removed_ids) {
        ...
        removed_site_min_buffer <- removed_site_sf %>% 
          st_transform(crs = st_crs(4326)) %>%#transform back to geographic to allow the use of the degree buffer
        ...
        lookup_site_sf_geo <- lookup_site_sf %>% st_transform(crs = st_crs(aeqd_proj))
        
        sites_near_removed_sites <- sites_near_removed_sites %>% 
          mutate(DistToRemovedSite = as.numeric(st_distance(removed_site_sf, sites_near_removed_sites, by_element=FALSE, which = "Hausdorff")), #Calculates the distance between all sites and the removed_site
                 DistToLookupSite = as.numeric(st_distance(lookup_site_sf_geo, sites_near_removed_sites, by_element=FALSE, which = "Hausdorff")))  #Calculates the distance between all sites and the lookup_site
